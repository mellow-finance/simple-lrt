...
contract Collector {
    ...
    mapping(address vault => bool) public isDVV;
    ...
    function setIsDVV(address vault, bool flag) external onlyOwner {
        isDVV[vault] = flag;
    }
    ...
    function getWithdrawals(MultiVault vault, uint256 assets)
        public
        view
    ...
    function getVaultAssets(
        MultiVault vault,
        address account,
        uint256 shares // for withdrawal request
    )
        public
        returns (
        ...
        )
    {
        ...
        uint256 instantAssets = 0;
        accountAssets = vault.previewRedeem(shares == 0 ? vault.balanceOf(account) : shares);
        if (isDVV[address(vault)]) {
            uint256 yieldVaultAssets = 0;
            MultiVault yieldVault = DVV(address(vault)).yieldVault();
            (instantAssets, yieldVaultAssets) = getVaultAssets(yieldVault, address(vault));
            withdrawals = getWithdrawals(yieldVault, Math.min(yieldVaultAssets, userAssets));
            instantAssets += IERC20(wsteth).balanceOf(address(vault));
            instantAssets +=
                IWSTETH(wsteth).getWstETHByStETH(IERC20(wsteth).balanceOf(address(vault)));
        } else {
            instantAssets = IERC20(asset).balanceOf(address(vault));
            address collateral = vault.defaultCollateral();
            if (collateral != address(0)) {
                instantAssets += IERC20(collateral).balanceOf(address(vault));
            }
            uint256 subvaultsCount = vault.subvaultsCount();
            for (uint256 i = 0; i < subvaultsCount; i++) {
                IMultiVaultStorage.Subvault memory subvault = vault.subvaultAt(i);
                if (subvault.withdrawalQueue == address(0)) {
                    instantAssets += IERC4626(subvault.vault).previewRedeem(
                        IERC4626(subvault.vault).balanceOf(address(vault))
                    );
                }
            }
            accountInstantAssets = Math.min(userAssets, instantAssets);
            withdrawals = getWithdrawals(vault, userAssets);
        }
    }
}
