...
interface IPermissionControllerEvents {
    ...
    event AppointeeSet(
        address indexed account, address indexed appointee, address target, bytes4 selector
    );
    ...
    ISemVerMixin
{
     ...
     */
    function canCall(address account, address caller, address target, bytes4 selector)
        external
        returns (bool);
     ...
     */
    function getAppointees(address account, address target, bytes4 selector)
        external
